...
class EventTest {
    ...
    class StaticLoaders {
        ...
        void testLoadAllEvents_ReturnsSeededEvents() {
            // ACT: Carichiamo tutti gli eventi
            List<Event> events = Event.loadAllEvents();

            // ASSERT: Verifichiamo che l'evento di seed sia stato caricato correttamente
            assertNotNull(events);
            assertFalse(events.isEmpty(), "Lo script di seed deve popolare almeno un evento.");
            
            Event sample = events.get(0);
            assertEquals("Gala Aziendale Annuale", sample.getName());
            assertNotNull(sample.getOrganizer(), "L'evento 'Gala' deve avere un organizzatore.");
            assertEquals("Francesca", sample.getOrganizer().getUserName());
            assertNotNull(sample.getChef(), "L'evento 'Gala' deve avere uno chef.");
            assertEquals("Antonio", sample.getChef().getUserName());
        }
