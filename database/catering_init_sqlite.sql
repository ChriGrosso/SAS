...
CREATE TABLE
    `Events` (
        ...
        `date_start` DATE,
        `date_end` DATE,
        `expected_persons` INTEGER, -- NUOVA COLONNA
        `location` TEXT,            -- NUOVA COLONNA
        `notes` TEXT,               -- NUOVA COLONNA
        `chef_id` INTEGER,          -- RESO NULLABILE (può essere assegnato dopo)
        `organizer_id` INTEGER NOT NULL -- NOME CORRETTO e obbligatorio
    );
...
-- Create a new event
INSERT INTO Events (name, date_start, date_end, expected_persons, location, notes, chef_id, organizer_id) VALUES 
('Gala Aziendale Annuale', date('2025-06-15'), date('2025-06-16'), 150, 'Salone delle Feste', 'Nessuna nota particolare', 5, 8);  -- Assigned to Antonio, organized by Francesca
